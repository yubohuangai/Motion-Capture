...
This document describes the **foundation motion capture** object-reconstruction path under `apps/reconstruction/`. It targets **calibrated multi-camera RGB** rigs (e.g. EasyMocap-style `intri.yml` / `extri.yml`), optional **foreground masks**, **COLMAP** export and dense MVS, and **3D Gaussian Splatting** training.

---

## Goals and assumptions

- **Input:** Synchronized images per camera (`data/images/<cam>/<frame>.jpg`), camera intrinsics and extrinsics (YAML), optional background-only frames for masking.
...
| `export_colmap.py` | Calibration → COLMAP; undistort; optional masks; sparse triangulation with **spatial neighbor** matching. |
...
Reduces background features in COLMAP and improves focus on the object.
...
**Outputs:** `data/masks/<cam>/000000.png` (and optional `mask_vis/` overlays).
...
1. Builds **PINHOLE** cameras (with `--undistort`: undistorted images and intrinsics).
2. Copies images as `01.jpg`, `02.jpg`, … and masks as `masks/01.jpg.png`, …
...
| Masks | `export_colmap.py --mask`, `dense_reconstruct.py --mask` | COLMAP feature extraction + dense image black-out. |
...
python apps/reconstruction/export_colmap.py $DATA \
  --intri $INTRI --extri $EXTRI --frame 0 --output $OUT/colmap \
  --undistort --mask masks --triangulate --gpu
