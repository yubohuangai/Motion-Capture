#!/usr/bin/env python3
"""
Copy a contiguous frame range from every ``NN/images/`` folder under a session ``raw/`` root.
...
Example (Alliance)::
    ...
    python scripts/preprocess/copy_ranges.py \\
...
def run_cli_clip(base_root: str, output_root: str, frame_start: int, frame_end: int) -> None:
    ...
    print(
        f"[copy_ranges] Input: {base_root}\n"
        f"[copy_ranges] Output: {output_root}/images/<cam>/\n"
        f"[copy_ranges] Frames (1-based, inclusive): {frame_start}..{frame_end} "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Copy a frame range from multi-camera raw/NN/images/ into a new dataset root."
    )
    parser.add_argument(
        "--input",
        "-i",
...
if __name__ == "__main__":
    ...
    if args.legacy_config:
        ...
        raise SystemExit(
            ...
            "\n"
            "Example:\n"
            "  python scripts/preprocess/copy_ranges.py -i ~/scratch/cow_2_board/raw "
